...
- [Lab 3：中断与中断处理流程](#lab-3中断与中断处理流程)
  - [实验内容](#实验内容)
  - [实验目的](#实验目的)
  - [练习1：完善中断处理](#练习1完善中断处理)
  - [扩展练习1：描述与理解中断流程](#扩展练习1描述与理解中断流程)
  - [扩增练习 Challenge2：理解上下文切换机制](#扩增练习-challenge2理解上下文切换机制)
  - [扩展练习 Challenge3：完善异常中断](#扩展练习-challenge3完善异常中断)
...
1.ucore处理异常大致分为以下几个阶段：

首先当cpu产生异常时，会保存当前中断发生时的pc值到sepc寄存器，记录中断异常的类型并将其写入scause寄存器中。同时，如果异常与缺页或者访问错误有关，会将相关的数据保存到stval寄存器，以便中断处理程序在后续过程中的使用。接着会保存修改中断使能状态和特权级，将当前的中断使能状态sstatus.SIE保存到sstatus.SPIE中，并将sstatus.SIE清零，将当前特权级信息保存到sstatus.SPP中，将pc设置为stvec寄存器中的值。
接着需要使用一个trapframe结构体来保存CPU的上下文信息到内存中，总共占据36个寄存器大小的空间，包含32个通用寄存器和4个和中断相关的CSR。我们可以定义一个汇编宏SAVE_ALL，来将这个结构体放到栈顶。保存完毕后跳转到trap函数入口处调用中断处理函数。
在中断处理过程中，trap函数根据interrupt_handler()和exception_handler()函数根据scause的值来处理中断和异常。处理结束后，通过恢复上下文的汇编宏RESTORE_ALL来恢复CPU的寄存器，先加载两个CSR，再加载31个通用寄存器，顺序与SAVE_ALL相反。

2.move a0, sp的目的：

按照RISC-V调用约定，a0寄存器用于传递第一个参数。这里将当前指向刚保存的trapframe结构体的栈指针作为参数传递给C函数trap()，使得C代码能够访问所有保存的寄存器值和CSR值。

3.SAVE_ALL中寄存器保存在栈中的位置确定

寄存器的位置由结构体trapframe的内存布局来确定，具体分配36个寄存器的连续内存空间，每个寄存器的偏移量为寄存器编号*寄存器大小（8字节）。

4.对于任何中断，__alltraps中都需要保存所有寄存器吗？

需要保留所有寄存器，首先中断处理程序无法预知那些寄存器被使用，系统可能需要访问不同的寄存器，而且任何终端都可能引发进程切换，所以需要完整地保存当前执行的上下文，新进程恢复时才能恢复完整的寄存器状态。

## 扩增练习 Challenge2：理解上下文切换机制

1.在trapentry.S中汇编代码 csrw sscratch, sp；csrrw s0, sscratch, x0实现了什么操作，目的是什么？

首先csrw sscratch, sp将中断前的原栈顶指针sp到sscratch中，当栈顶指针向低地址空间眼神36个寄存器空间后，csrrw s0, sscratch, x0再将存放在sscratch中的原sp指针写到内存中，只不过CSR中的值不能直接写到内存，必须先读取到通用寄存器才能写到内存，目的就是为了保存原始栈顶指针，为后续处理准备。

2.save all里面保存了stval scause这些csr，而在restore all里面却不还原它们？那这样store的意义何在呢？

像scause（异常原因寄存器）、stval（陷入值寄存器）等不需要恢复，因为它们对应的是此次中断的值，是要为中断处理程序提供完整的上下文，但当此次中断已经处理完毕后，这些旧值对返回后的执行结果毫无意义。
...
100 ticks
```
我们可以看到他正常输出了异常指令的地址0xc020009c和0xc02000a0，以及每隔大约1秒输出一个100tick，总共十个。
