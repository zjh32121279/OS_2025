...
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
               bool share)// 复制 from 中的页表项到 to 中   
{
    assert(start % PGSIZE == 0 && end % PGSIZE == 0);
    assert(USER_ACCESS(start, end));
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
             ...
             */
            // (1) find src_kvaddr: the kernel virtual address of page
            void *src = page2kva(page);  // 应该是 void* 类型// 找到page的内核虚拟地址
            // (2) find dst_kvaddr: the kernel virtual address of npage  
            void *dst = page2kva(npage); // 应该是 void* 类型// 找到npage的内核虚拟地址
            // (3) memory copy from src_kvaddr to dst_kvaddr, size is PGSIZE
            memcpy(dst, src, PGSIZE); // 从src_kvaddr复制PGSIZE字节到dst_kvaddr
            // (4) build the map of phy addr of nage with the linear addr start
            ret = page_insert(to, npage, start, perm); // 建立npage与start的映射关系
            assert(ret == 0);
        }
        start += PGSIZE;
    } while (start != 0 && start < end);
